...
terraform {
  backend "s3" {
    ...
    key            = "timeclone/pod-DEVOPS/aws/staging/rds/postgres_supplier/terraform.tfstate"
    region         = "us-east-1"
...
data "aws_secretsmanager_secret_version" "get_rds" {
  secret_id = data.terraform_remote_state.devops_staging_secretmanager.outputs.secrets_manager_rds_postgres_supplier.id
}
