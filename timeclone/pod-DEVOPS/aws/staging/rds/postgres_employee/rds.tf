...
resource "cloudflare_record" "endpoint" {
  ...
  name    = module.rds_postgres_employee.rds_identifier
  value   = module.rds_postgres_employee.instance_address
  type    = "CNAME"
}
...
output "rds_endpoint" {
  ...
  sensitive   = true
  value       = "${module.rds_postgres_employee.rds_identifier}.${local.domain}"
}
